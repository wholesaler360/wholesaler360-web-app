...
const refreshAccessToken = asyncHandler(async (req, res, next) => {
  ...
  console.log("Testing Refresh Token", refreshToken);
  if (!refreshToken) {
  ...
  try {
    ...
    if (!user || user?.refreshToken !== refreshToken) {
      ...
      return next(
        ApiError.unauthorizedAccess("User Does Not Exists or Login Again")
      );
    }
    ...
    // TODO : Redirect to login page
    return next(
      ApiError.dataNotInserted("Unable to generate new access Token")
    );
  }
});
...
const logout = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (!loggedOutUser) {
      return next(
        ApiError.validationFailed("User not found or already logged out")
      );
    }
